...
{
	static const guchar data[] = { 0x00, 0x01 };
	char* hex = ril_encode_hex(NULL, 0);
	void *out;
	guint size = 1;
	...
	g_assert(!ril_decode_hex(NULL, 0, NULL));

	hex = ril_encode_hex(data, sizeof(data));
	g_assert_cmpstr(hex, == ,"0001");

	g_assert(!ril_decode_hex(hex, 0, &size));
	g_assert(!ril_decode_hex(hex, 1, &size));
	g_assert(!ril_decode_hex("..", -1, &size));
	g_assert_cmpuint(size, == ,0);

	out = ril_decode_hex(hex, -1, &size);
	g_assert(out);
	g_assert_cmpuint(size, == ,2);
	g_assert(!memcmp(out, data, size));
...
int main(int argc, char *argv[])
{
	...
	g_test_add_func(TEST_("hex"), test_hex);
